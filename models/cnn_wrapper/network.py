...
def caffe_like_padding(input_tensor, padding):
    ...
    if len(input_tensor.get_shape()) == 4:
        padded_input = tf1.pad(input_tensor,
...
def layer(op):
    ...
    def layer_decorated(self, *args, **kwargs):
        ...
        if self.inputs is None or (len(args) > 0 and isinstance(args[0], tf1.Tensor)):
...
class Network(object):
    ...
    def __init__(self, inputs, is_training,
        ...
        #self.regularizer = tf1.contrib.layers.l2_regularizer(1.0) if regularize else None
        self.regularizer = tf1.keras.regularizers.L2(1.0) if regularize else None
    ...
    def load(self, data_path, session, ignore_missing=False, exclude_var=None):
        ...
        for op_name in data_dict:
            ...
            with tf1.variable_scope(op_name, reuse=True):
                for param_name, data in data_dict[op_name].items():

                    try:
                        var = tf1.get_variable(param_name)
    ...
    def conv(self,
             ...
             reuse=False,
             kernel_init=None,
             bias_init=tf1.zeros_initializer,
             separable=False):
        """2D/3D convolution."""
        kwargs = {'filters': filters,
                  'kernel_size': kernel_size,
                  'strides': strides,
                  'activation': tf1.nn.relu if relu else None,
                  'use_bias': biased,
        ...
        if len(input_tensor.get_shape()) == 4:
            if not separable:
                return tf1.compat.v1.layers.conv2d(padded_input, **kwargs)
            else:
                return tf1.layers.separable_conv2d(padded_input, **kwargs)
        elif len(input_tensor.get_shape()) == 5:
            if not separable:
                return tf1.layers.conv3d(padded_input, **kwargs)
            else:
    ...
    def deconv(self,
        ...
        if len(input_tensor.get_shape()) == 4:
            return tf1.layers.conv2d_transpose(padded_input, **kwargs)
        elif len(input_tensor.get_shape()) == 5:
            return tf1.layers.conv3d_transpose(padded_input, **kwargs)
        else:
            raise ValueError('Improper input rank for layer: ' + name)
    ...
    def relu(self, input_tensor, name=None):
        """ReLu activation."""
        return tf1.nn.relu(input_tensor, name=name)
    ...
    def max_pool(self, input_tensor, pool_size, strides, name, padding=DEFAULT_PADDING):
        ...
        return tf1.layers.max_pooling2d(padded_input,
                                       pool_size=pool_size,
                                       strides=strides,
    ...
    def avg_pool(self, input_tensor, pool_size, strides, name, padding=DEFAULT_PADDING):
        ...
        if isinstance(padding, str):
            ...
            padded_input = caffe_like_padding(input_tensor, padding)
            padding_type = 'VALID'
        return tf1.layers.average_pooling2d(padded_input,
                                           pool_size=pool_size,
                                           strides=strides,
                                           padding=padding_type,
                                           name=name)
    ...
    def concat(self, input_tensors, axis, name):
        return tf1.concat(values=input_tensors, axis=axis, name=name)

    @layer
    def add(self, input_tensors, name):
        return tf1.add_n(input_tensors, name=name)

    @layer
    def fc(self, input_tensor, num_out, name, biased=True, relu=True, flatten=True, reuse=False):
        # To behave same to Caffe.
        if flatten:
            flatten_tensor = tf1.layers.flatten(input_tensor)
        else:
            flatten_tensor = input_tensor
        return tf1.layers.dense(flatten_tensor,
                               units=num_out,
                               use_bias=biased,
                               activation=tf1.nn.relu if relu else None,
                               trainable=self.trainable,
    ...
    def softmax(self, input_tensor, name, dim=-1):
        return tf1.nn.softmax(input_tensor, dim=dim, name=name)

    @layer
    def batch_normalization(self, input_tensor, name,
                            center=False, scale=False, relu=False, reuse=False):
        """Batch normalization."""
        output = tf1.layers.batch_normalization(input_tensor,
                                               center=center,
                                               scale=scale,
                                               fused=True,
    ...
    def context_normalization(self, input_tensor, name):
        ...
        mean, variance = tf1.nn.moments(input_tensor, axes=[1], keep_dims=True)
        output = tf1.nn.batch_normalization(
            input_tensor, mean, variance, None, None, self.bn_epsilon)
        return output

    @layer
    def l2norm(self, input_tensor, name, axis=-1):
        return tf1.nn.l2_normalize(input_tensor, axis=axis, name=name)

    @layer
    def squeeze(self, input_tensor, axis=None, name=None):
        return tf1.squeeze(input_tensor, axis=axis, name=name)

    @layer
    def reshape(self, input_tensor, shape, name=None):
        return tf1.reshape(input_tensor, shape, name=name)

    @layer
    def flatten(self, input_tensor, name=None):
        return tf1.layers.flatten(input_tensor, name=name)

    @layer
    def tanh(self, input_tensor, name=None):
        return tf1.tanh(input_tensor, name=name)
    ...
    def deform_conv(self,
        ...
        def _get_conv_indices(feature_map_size, kernel_size, strides, dilation_rate):
            ...
            x, y = tf1.meshgrid(tf1.range(feat_w), tf1.range(feat_h))
            x, y = [tf1.reshape(i, [1, feat_h, feat_w, 1]) for i in [x, y]]  # shape [1, h, w, 1]
            x, y = [tf1.image.extract_patches(i,
        ...
        # some length
        batch_size = tf1.shape(inputs)[0]
        if self.training:
            in_h, in_w = [inputs.get_shape()[i].value for i in range(1, 3)]
            ori_h, ori_w = [input_tensor.get_shape()[i].value for i in range(1, 3)]
        else:
            in_h, in_w = [tf1.shape(inputs)[i] for i in range(1, 3)]
            ori_h, ori_w = [tf1.shape(input_tensor)[i] for i in range(1, 3)]

        with tf1.compat.v1.variable_scope('deform_param', reuse=reuse):
            if deform_type == 'a':
                # similarity est
                ori = self.conv(inputs, kernel_size, 2, strides, name + '/ori', relu=False,
                                dilation_rate=dilation_rate, padding='VALID',
                                kernel_init=tf1.zeros_initializer,
                                bias_init=tf1.constant_initializer([1., 0.]),
                                biased=True, reuse=reuse)
                ori = tf1.nn.l2_normalize(ori, axis=-1)
                cos = ori[:, :, :, 0, None]
                sin = ori[:, :, :, 1, None]
                R = tf1.stack([tf1.concat([cos, sin], axis=-1),
                              tf1.concat([-sin, cos], axis=-1)], axis=-2)
                scale = self.conv(inputs, kernel_size, 1, strides, name + '/scale', relu=False,
                                  dilation_rate=dilation_rate, padding='VALID',
                                  kernel_init=tf1.zeros_initializer,
                                  bias_init=tf1.constant_initializer(0.),
                                  biased=True, reuse=reuse)
                scale = tf1.exp(tf1.tanh(scale))
                comb_A = scale[..., None] * R
                # aff est
                if False:
                ...
                rng = tf1.range(-(kernel_size // 2), kernel_size // 2 + 1)
                x, y = tf1.meshgrid(rng, rng)
                x = tf1.reshape(x, (-1, ))
                y = tf1.reshape(y, (-1, ))
                xy = tf1.reshape(tf1.stack([x, y], axis=-1), [1, 1, 1, -1, 2])
                xy = tf1.tile(xy, [tf1.shape(comb_A)[0], tf1.shape(
                    comb_A)[1], tf1.shape(comb_A)[2], 1, 1])
                xy = tf1.cast(xy, tf1.float32)

                offset = tf1.matmul(xy, comb_A) - xy
                offset = tf1.stack([offset[:, :, :, :, 1], offset[:, :, :, :, 0]], axis=-1)
                if self.training:
                    offset = tf1.reshape(offset, [batch_size, comb_A.get_shape()[1].value,
                                                 comb_A.get_shape()[2], offset_num * 2])
                else:
                    offset = tf1.reshape(offset, [batch_size, tf1.shape(
                        comb_A)[1], tf1.shape(comb_A)[2], offset_num * 2])
            elif deform_type == 'h':
                h4p_offset = self.conv(inputs, kernel_size, 8, strides, name + '/offset', relu=False,
                                       dilation_rate=dilation_rate, padding='VALID',
                                       kernel_init=tf1.zeros_initializer, bias_init=tf1.zeros_initializer,
                                       biased=True, reuse=reuse)
                ...
                y = tf1.reshape(y, (-1, ))
                xy = tf1.reshape(tf1.stack([x, y], axis=-1), [1, 1, 1, -1, 2])
                xy = tf1.tile(xy, [tf1.shape(H_mat)[0], tf1.shape(
                    H_mat)[1], tf1.shape(H_mat)[2], 1, 1])
                xy = tf1.cast(xy, tf1.float32)
                ones = tf1.ones_like(xy[:, :, :, :, 0])[..., None]
                xy_homo = tf1.concat([xy, ones], axis=-1)

                pert_xy = tf1.matmul(xy_homo, H_mat, transpose_b=True)
                homo_scale = tf1.expand_dims(pert_xy[:, :, :, :, -1], axis=-1)
                pert_xy = pert_xy[:, :, :, :, 0:2]
                pert_xy = tf1.clip_by_value(tf1.math.divide_no_nan(pert_xy, homo_scale), -10., 10.)

                offset = pert_xy - xy
                offset = tf1.stack([offset[:, :, :, :, 1], offset[:, :, :, :, 0]], axis=-1)
                if self.training:
                    offset = tf1.reshape(offset, [batch_size, H_mat.get_shape()[1].value,
                                                 H_mat.get_shape()[2], offset_num * 2])
                else:
                    offset = tf1.reshape(offset, [batch_size, tf1.shape(
                        H_mat)[1], tf1.shape(H_mat)[2], offset_num * 2])
            elif deform_type == 'u':
            ...
            if modulated:
                amplitude = self.conv(inputs, kernel_size, offset_num, strides, name + '/amplitude', relu=False,
                                      ...
                                      kernel_init=tf1.zeros_initializer, bias_init=tf1.zeros_initializer,
                                      biased=True, reuse=reuse)
                amplitude = tf1.math.sigmoid(amplitude)

        if self.training:
            out_h, out_w = [offset.get_shape()[i].value for i in range(1, 3)]
        else:
            out_h, out_w = [tf1.shape(offset)[i] for i in range(1, 3)]
        # get x, y axis offset
        offset = tf1.reshape(offset, [batch_size, out_h, out_w, offset_num, 2])
        y_off, x_off = offset[:, :, :, :, 0], offset[:, :, :, :, 1]

        # input feature map gird coordinates
        y, x = _get_conv_indices([in_h, in_w], kernel_size, strides, dilation_rate)
        y, x = [tf1.reshape(i, [batch_size, out_h, out_w, offset_num]) for i in [y, x]]
        y, x = [tf1.cast(i, tf1.float32) for i in [y, x]]

        # add offset
        y, x = y + y_off, x + x_off
        y = tf1.clip_by_value(y, 0, tf1.cast(in_h, tf1.float32) - 1)
        x = tf1.clip_by_value(x, 0, tf1.cast(in_w, tf1.float32) - 1)

        # get four coordinates of points around (x, y)
        y0, x0 = [tf1.cast(tf1.floor(i), tf1.int32) for i in [y, x]]
        y1, x1 = y0 + 1, x0 + 1
        # clip
        y0, y1 = [tf1.clip_by_value(i, 0, in_h - 1) for i in [y0, y1]]
        x0, x1 = [tf1.clip_by_value(i, 0, in_w - 1) for i in [x0, x1]]

        # get pixel values
        indices = [[y0, x0], [y0, x1], [y1, x0], [y1, x1]]
        p0, p1, p2, p3 = [tf1.gather_nd(inputs, tf1.stack(i, axis=-1), batch_dims=1) for i in indices]

        # cast to float
        x0, x1, y0, y1 = [tf1.cast(i, tf1.float32) for i in [x0, x1, y0, y1]]
        # weights
        w0 = (y1 - y) * (x1 - x)
        w1 = (y1 - y) * (x - x0)
        w2 = (y - y0) * (x1 - x)
        ...
        w0, w1, w2, w3 = [tf1.expand_dims(i, axis=-1) for i in [w0, w1, w2, w3]]
        # bilinear interpolation
        pixels = tf1.add_n([w0 * p0, w1 * p1, w2 * p2, w3 * p3])

        if modulated:
            pixels *= tf1.expand_dims(amplitude, -1)
        ...
        if False:
            ...
            with tf1.compat.v1.variable_scope(name, reuse=self.reuse):
                weights = tf1.compat.v1.get_variable(
                    'kernel', (kernel_size, kernel_size, c, filters), dtype=tf1.float32, initializer=kernel_init)
                weights = tf1.reshape(weights, (1, 1, kernel_size ** 2, c, filters))
                out = tf1.nn.conv3d(input=pixels, filters=weights, strides=[
                    1, 1, 1, 1, 1], padding='VALID', name=None)
                out = tf1.squeeze(out, axis=-2)

            if biased:
                bias = tf1.compat.v1.get_variable(
                    'bias', (filters, ), dtype=tf1.float32, initializer=bias_init)
                out = tf1.nn.bias_add(out, bias)
